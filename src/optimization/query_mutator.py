...
class QueryMutator:
    ...
    def mutate(
        ...
        **context
        ...
        try:
            mutated_query = self.llm.generate(
                prompt,
                max_new_tokens=1024,  # ← improve to 1024（512 is too small）
                temperature=0.7
            )
            
            # Post-process: strip extra whitespace
            mutated_query = self._post_process(mutated_query, query)
    ...
    def _post_process(self, mutated_query: str, original_query: str) -> str:
        """
        Post-process mutated query
        
        Fixes:
        1. Remove "Output:" prefix (if LLM echoed it)
        2. Detect and fix truncation
        3. Ensure complete sentences
        """
        # 1. Remove common prefixes
        prefixes_to_remove = ["Output:", "output:", "**Output:**", "A:", "Answer:"]
        for prefix in prefixes_to_remove:
            if mutated_query.startswith(prefix):
                mutated_query = mutated_query[len(prefix):].strip()
        
        # 2. Detect truncation
        if self._is_truncated(mutated_query):
            logger.warning(f"Detected truncated query: '{mutated_query[-50:]}'")
        ...
        # 4. If empty, fallback
        if not mutated_query:
            logger.warning("Empty mutated query, using original")
            return original_query
    ...
    def _is_truncated(self, text: str) -> bool:
        ...
        Heuristics:
        1. Ends with incomplete word (lowercase letter without punctuation)
        2. Ends with common incomplete tokens: "the", "a", "an", "of", "to", "in"
        3. Ends with "**" or other markdown artifacts
        4. Last sentence is incomplete (no punctuation in last 20 chars)
        """
        if not text:
            return True
        ...
        # Check 1: Ends with incomplete markdown
        if text.endswith(("**", "*", "`", "``")):
            return True
        
        # Check 2: Ends with dangling preposition/article
        last_words = text.split()[-3:]  # Last 3 words
        dangling_words = {"the", "a", "an", "of", "to", "in", "on", "at", "by", "for", "with", "through"}
        if last_words and last_words[-1].lower() in dangling_words:
            return True
        
        # Check 3: No punctuation in last 20 characters (incomplete sentence)
        # last_20 = text[-20:] if len(text) > 20 else text
        # if not any(p in last_20 for p in ".!?"):
        #     # But allow if it's a short query (< 50 chars)
        #     if len(text) > 50:
        #         return True
