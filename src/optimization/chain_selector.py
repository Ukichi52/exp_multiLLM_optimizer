...
class ChainSelector:
    ...
    def _load_discovered_chains(self):
        """Load LLM-discovered or manual chains"""
        import json
        from pathlib import Path
        
        discovered_path = Path("discovered_chains.json")
        if discovered_path.exists():
            with open(discovered_path) as f:
                discovered = json.load(f)
            
            for chain_info in discovered:
                self.predefined_chains[chain_info['chain_name']] = chain_info['chain']
            
            logger.info(f"✅ Loaded {len(discovered)} discovered chains")
        else:
            logger.info("No discovered_chains.json found, using defaults only")
    ...
    def select_chain(
        ...
        if self.analytics:
            # 获取成功率最高的 chain
            top_chains = self.analytics.get_top_chains(n=3)
            
            if top_chains and top_chains[0][1] > 0.5:  # 成功率 > 50%
                best_chain_name = top_chains[0][0]
                
                logger.info(
                    f"Using analytics: {best_chain_name} "
                    f"(success rate: {top_chains[0][1]:.1%})"
                )
                
                return {
                    'chain_name': best_chain_name,
                    'chain': self.predefined_chains[best_chain_name],
                    'reason': f'Analytics (success rate: {top_chains[0][1]:.1%})',
                    'confidence': top_chains[0][1],
                    'method': 'analytics'
                }
...
# Add this to your ChainSelector or create EnhancedChainSelector

import json
from pathlib import Path

class EnhancedChainSelector(ChainSelector):
    def __init__(self, strategy_pool, predefined_chains=None, analytics=None):
        super().__init__(strategy_pool, predefined_chains, analytics)
        
        # Load discovered chains
        discovered_path = Path("discovered_chains.json")
        if discovered_path.exists():
            with open(discovered_path) as f:
                discovered = json.load(f)
